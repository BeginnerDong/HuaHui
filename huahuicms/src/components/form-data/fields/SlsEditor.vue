  ...
  Js.mixins = [{
    data () {
      return {
        ...
        attrs: {
          ...
          uploadImgServer: 'http://solelytech.iicp.net/yts/public/api/v1/Base/FtpImage/upload', // 上传图片到服务器
          ...
          }, // 上传图片时可自定义传递一些参数，例如传递验证的token等。这些参数会拼接到 url 的参数中，也会被添加到formdata中。
          uploadFileName: 'file', // 上传图片时，可自定义filename，即在使用formdata.append(name, file)添加图片文件时，自定义第一个参数。
    ...
    methods: {
      ...
      initWangEditor () {
        ...
        this.editor.customConfig.uploadImgHooks = Object.assign(this.editor.customConfig.uploadImgHooks, this.event)
